...
char* registerToBitString(uint8_t reg);
...
const uint8_t current_reset_pin = 7; //Treba postavit
...
uint16_t zero_cross_correction = 600;
uint16_t stability_boundarie = 5600;
...
Timer.setupTimer();
pinMode(current_reset_pin, OUTPUT);
}
...
void loop()
{
 /* //PID controller
  ...
  if (Serial.available() > 0) {
    ...
    data = Serial.read();
    timer_delay = zero_cross_correction + rmsToTimeDelay(data);
  }
...
void zeroCross()
{
  ...
  Timer.setTimerDelay(timer_delay);
...
void setupInterruptPin(const int &pin, void(*pFuncton)())
{
  ...
  Serial.println("Interrupt attached to pin: ");
  Serial.print(pin);
  Serial.print("\n");
}
...
void setupTriacPin(const uint8_t &pin)
{
  ...
  Serial.print("Triac attached to pin: ");
  Serial.print(pin);
  Serial.print("\n");
}
...
uint16_t rmsToTimeDelay(uint16_t target_value)
{
  ...
  const FunctionPair Phase_Angle_To_Time_Delay[] =
    ...
    {
      if((Phase_Angle_To_Time_Delay[counter].output - target_value) <= 0)
      {
        index = counter;
        //Serial.println("Jack pot**********************");
      } 
    }
